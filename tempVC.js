	...
	*/
	...
	if (config.enable_slash) {
		...
		const cmd1 = new SlashCommandBuilder()
			.setName('vc-fix')
			.setDescription('Fix TempVCs')
			.setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
		const cmd2 = new SlashCommandBuilder()
			.setName('vc-hubmsg')
			.setDescription('Create a HUB message')
			.setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
		const cmd3 = new SlashCommandBuilder()
			.setName('vc-control')
			.setDescription('Call the console')
	...
	*/
	...
	if (!newUserChannel || newUserChannel != tempVcId) {
		if (typeof tempVcId !== 'undefined') {
			...
			ownerLeave_Embed.setDescription(`The owner has left <#${tempVcId}>。 \nThe channel will be available in <t:${Math.floor(Date.now() / 1000) + 180}:R> Delete. \nTo cancel, rejoin the voice channel.`)
			let Vc = await guild.channels.cache.find(channel => channel.id == tempVcId)
			if (!Vc) {
				await db.del(`tempVcIdKey_${userid}`).catch(err => { console.log('Error kv!') })
				await db.del(`tempmsgIdKey_${userid}`).catch(err => { console.log('Error kv!') })
				await db.del(`tempTimestampKey_${userid}`).catch(err => { console.log('Error kv!') })
				return
			}
	...
	if (interaction.isRoleSelectMenu() || interaction.isUserSelectMenu()) {
		if (interaction.customId == "menu_whitelist_member" || interaction.customId == "menu_whitelist_role") {
			...
			const roleselectmenu = new RoleSelectMenuBuilder()
				...
				.setPlaceholder('Select an identity group')
		...
		if (interaction.customId === 'modal_limit') {
			...
			return interaction.editReply({ content: `OK. VC name is set to ${inputstr}.\nOk, the name has been changed to${inputstr}.`, ephemeral: true }).catch(err => { VC.send(`Something is wrong. Please try again.\nError catch: \n\`\`\` ${err}\n\`\`\``) })
		...
		if (interaction.customId === "button_lock") {
			...
			if (removeTimeouts2[user.id]) {
				...
				predelete_Embed.setDescription(`The homeowner decided to will<#${tempVcId}>Delete. \nThe channel will be available in <t:${Math.floor(Date.now() / 1000) + 10}:R> Delete. \nTo cancel, press DeleteVC again.`)
		...
		if (interaction.commandName == "vc-fix") {
			...
			for await (const key of db.keys()) {
				if (key.startsWith('tempVcIdKey_')) {
					...
					let Vc = await guild.channels.cache.find(channel => channel.id == tempVcId)
					adminDeleteEmbed.setDescription(`The channel will be available in <t:${Math.floor(Date.now() / 1000) + 10}:R> Delete.`)
					Vc.send({ content: `<@${uid}>`, embeds: [adminDeleteEmbed] })
					setTimeout(RemoveVTChannels, 10000, uid, tempVcId)
				}
			}
			return await interaction.editReply({ content: 'OK. Self-detonation start.', ephemeral: true })
		}
